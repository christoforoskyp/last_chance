...
BASE = "http://127.0.0.1:5000/"  # poustra's ip
IP = "http://127.0.0.1:"
urls = {}
...
class Home(Resource):  # first thing that happens
    def get(self):
        my_id = requests.get(BASE + "create_id/5004")  # send to NODE_0 my port and get my id
        global node
        node = Node.Node(my_id.json(), [0])  # create my Node, with the id NODE_0 gave me
        node.generate_wallet()
        requests.post(BASE + "get_public_key/" + str(node.id),
